...
local getcustomassets = {
	...
	['vectorclient/assets/new/hit1.ogg'] = 'https://raw.githubusercontent.com/Loqrp/vectorclient/main/hit1.ogg',
	['vectorclient/assets/new/hit2.ogg'] = 'https://raw.githubusercontent.com/Loqrp/vectorclient/main/hit2.ogg',
	['vectorclient/assets/new/hit3.ogg'] = 'https://raw.githubusercontent.com/Loqrp/vectorclient/main/hit3.ogg'
}
...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/Loqrp/vectorclient/'..readfile('vectorclient/profiles/commit.txt')..'/'..select(1, path:gsub('vectorclient/', '')), true)
		end)
...
run(function()
	...
	local targetinfoobj
	local targetinfobcolor
	local targetinfodisplay


	local customGuiFrame = G2L["3"]
	local customPlayerImage = G2L["4"]
	local customHealthFrame = G2L["5"]
	local customUsernameLabel = G2L["9"]


	customGuiFrame.Visible = false
	...
	targetinfo = {
		...
		UpdateInfo = function(self)
			...
			customGuiFrame.Visible = v ~= nil or (mainapi.gui and mainapi.gui.ScaledGui and mainapi.gui.ScaledGui.ClickGui and mainapi.gui.ScaledGui.ClickGui.Visible)
			if v then
				customUsernameLabel.Text = v.Player and (targetinfodisplay and targetinfodisplay.Enabled and v.Player.DisplayName or v.Player.Name) or v.Character and v.Character.Name or "Unknown"
				local userId = v.Player and v.Player.UserId or 1
				customPlayerImage.Image = 'rbxthumb://type=AvatarHeadShot&id='..userId..'&w=420&h=420'
				...
				if v.Health ~= lasthealth or v.MaxHealth ~= lastmaxhealth then
					local percent = math.max(v.Health / v.MaxHealth, 0)
					local targetWidth = math.floor(129 * math.min(percent, 1))
					tween:Tween(customHealthFrame, TweenInfo.new(0.3), {
						Size = UDim2.new(0, targetWidth, customHealthFrame.Size.Y.Scale, customHealthFrame.Size.Y.Offset)
					})
					lasthealth = v.Health
					lastmaxhealth = v.MaxHealth
				end
