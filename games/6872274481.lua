...
run(function()
    ...
    local function loadFirstPack()
        ...
        local success, objs = pcall(game.GetObjects, game, "rbxassetid://13783192680")
        if success and type(objs) == "table" and #objs > 0 and objs[1]:IsA("Model") then
            importModel = objs[1]
    ...
    TexturePack = vape.Categories.Render:CreateModule({
        ...
        Function = function(callback)
            if callback then
                TexturePack.Enabled = true
                toolFunction = processTool
                loadFirstPack()
                refresh()
            else
                TexturePack.Enabled = false
        ...
        Tooltip = "Applies texture pack textures."
    })
